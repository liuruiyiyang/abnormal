...
from single import get_single_feature
...
def fit_window(df, window):
    df.drop(range(int(window / 2)), inplace=True)
    df.drop(range(len(df) - int(window / 2), len(df)), inplace=True)
...
# KPI_ID_e = ['07927a9a18fa19ae'] # , '76f4550c43334374' 07927a9a18fa19ae
...
for KPI_ID_name in KPI_ID_e:
    ...
    single_predict = get_single_feature(raw_df=KPI_LIST[index], KPI_ID_name=KPI_ID_name)
    single_predict = fit_window(single_predict, window)

    manual_feature_df = get_manual_feature(KPI_LIST[index])
    manual_feature_df = fit_window(df=manual_feature_df, window=window)
    ...
    X_train_df = pd.concat([X_train_df, manual_feature_df, train_processd_feature, single_predict], axis=1)
    ...
    pipe = make_pipeline_imb(MinMaxScaler(),
                             RandomUnderSampler(),
    ...
    single_predict = get_single_feature(raw_df=KPI_LIST_test[index], KPI_ID_name=KPI_ID_name)
    single_predict = fit_window(single_predict, window)
    ...
    X_train_df = pd.concat([X_train_df, test_manual_feature, test_processd_feature, single_predict], axis=1)
    # print("X_train_df of test:\n", X_train_df)
    X_train = X_train_df.values

    print("X_train.shape:", X_train.shape)

    y_test_preds = pipe.predict(X_train)

    # print("y_test_preds:")
    # print(y_test_preds)

    ran_predict = padding_shift_y(y_test_preds, window)
    ...
    ran_ts_result = get_result(ts_KPI_ID_test, ts_timestamp, ran_predict)
    print("xgb_ts_result:", ts_KPI_ID_test.iloc[[0]])

    save(result_path=rand_result_path, ts_result=ran_ts_result)
